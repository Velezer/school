@props(['assessmentUpdate' => [], 'periods', 'classrooms', 'subjects'])
                ...
                <div class="card-body">
                    ...
                    <div class="app-brand justify-content-center">
                        <a href="index.html" class="app-brand-link gap-2">
                            ...
                            <span class="app-brand-text demo text-body fw-bolder">Assessment</span>
                        </a>
                    </div>
                    <!-- /Logo -->
                    <h4 class="mb-2">Edit Assessment 🚀</h4>
                    <p class="mb-4">Silakan ubah form di bawah ini</p>

                    <form id="formUpdate" class="mb-3"
                        action="{{ $method == 'PUT' ? route('apiUpdateAssessment', $assessmentUpdate['id']) : route('apiCreateAssessment') }}"
                        method="POST">
                        @csrf
                        @if ($method == 'PUT')
                            @method('PUT')
                        ...
                        <div class="mb-3 row">
                            ...
                            <div class="col-md-10">
                                <input type="text" class="form-control" id="name" name="name"
                                    ...
                                    value="{{ $assessmentUpdate['name'] ?? '' }}" @required(true)/>
                            </div>
                        </div>

                        <div class="mb-3 row">
                            <label for="start_at" class="col-md-2 col-form-label">Mulai</label>
                            <div class="col-md-10">
                                <input class="form-control" type="datetime-local" id="start_at" name="start_at"
                                    value="{{ $assessmentUpdate['start_at'] ?? '' }}" @required(true)>
                            </div>
                        </div>
                        <div class="mb-3 row">
                            <label for="end_at" class="col-md-2 col-form-label">Selesai</label>
                            <div class="col-md-10">
                                <input class="form-control" type="datetime-local" id="end_at" name="end_at"
                                    value="{{ $assessmentUpdate['end_at'] ?? '' }}" @required(true)>
                            </div>
                        </div>
                                ...
                                <select id="subject" name="subject" class="form-select" aria-label="subject" required>
                                    ...
                                    @foreach ($subjects as $subject)
                                        <option value="{{ $subject }}"
                                            {{ $assessmentUpdate['subject'] ?? '' == $subject ? 'selected' : '' }}>
                                            {{ $subject }}</option>
                                ...
                                <select id="classroom" name="classroom" class="form-select" aria-label="classroom"
                                    ...
                                    @foreach ($classrooms as $classroom)
                                        ...
                                        <option value="{{ $classroom }}"
                                            {{ $assessmentUpdate['classroom'] ?? '' == $classroom ? 'selected' : '' }}>
                                            {{ $classroom }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="col-md-10">
                                ...
                                <select id="period" name="period" class="form-select" aria-label="period"
                                    ...
                                    @foreach ($periods as $period)
                                        <option value="{{ $period }}"
                                            {{ $assessmentUpdate['period'] ?? '' == $period ? 'selected' : '' }}>
                                            {{ $period }}</option>
